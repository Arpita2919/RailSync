...
const SCENARIO_PRESETS = [
  {
    label: 'Track Weld Defect at 15:30 (Active NCR Urgent)',
    type: 'track_weld_defect',
    segment_id: 'KRJ-UP-01',
    severity: 'critical',
    time: '15:30',
    description: 'Track weld defect detected under ultrasonic inspection at Khurja UP Mainline.',
  },
  {
    label: 'OHE Cantilever Sag at Post Km 1074/12',
    type: 'ohe_cantilever_sag',
    segment_id: 'SOM-DN-02',
    severity: 'moderate',
    time: '14:00',
    description: 'OHE overhead cantilever droop detected between Somna and Kulwa.',
  },
  {
    label: 'Signal Interlocking Electronic Failure (KRJ North)',
    type: 'signal_interlocking_failure',
    segment_id: 'KRJ-UP-04',
    severity: 'emergency',
    time: '16:00',
    description: 'Point machine 24B feedback failure at Khurja North cabin.',
  },
  {
    label: 'Custom Operator Injection (Manual Possessions)',
    type: 'emergency_inspection',
    segment_id: 'ALJN-UP-01',
    severity: 'critical',
    time: '12:00',
...
export default function WhatIfSandbox() {
  ...
  const [selectedSegmentId, setSelectedSegmentId] = useState(SCENARIO_PRESETS[0].segment_id);
  ...
  const handlePresetChange = (idx) => {
    ...
    if (segments.some(s => s.segment_id === p.segment_id)) {
  ...
  const handleReOptimize = async () => {
    ...
    try {
      const payload = {
        ...
        segment_id: selectedSegmentId || (segments[0]?.segment_id || 'KRJ-UP-01'),
                  ...
                  {segments.length > 0 ? (
                    segments.map((s) => (
                      <option key={s.segment_id} value={s.segment_id}>
                        {s.segment_id} — {s.station_from} to {s.station_to} ({s.line_type})
                      </option>
                    ))
                  ) : (
                    <option value="KRJ-UP-01">KRJ-UP-01 — UP Mainline</option>
                  )}
                </select>
                <span className="material-symbols-outlined absolute right-2 top-2 text-[16px] text-on-surface-variant pointer-events-none">
                  arrow_drop_down
