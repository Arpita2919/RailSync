...
DATA_CANDIDATES = [Path("data"), Path("."), Path("/content/data"), Path("/content")]
DATA_DIR = next(
...
def read_inputs():
    ...
    # Basic schema checks against the supplied Layer 0 contract.
    if len(seg) != 50:
        raise ValueError(f"Expected 50 segments, found {len(seg)}")
    if panel["segment_id"].nunique() != 50:
        raise ValueError("Monthly panel does not contain all 50 segments")
    if panel["month"].nunique() != 24:
        raise ValueError(f"Expected 24 monthly observations, found {panel['month'].nunique()}")
...
def train_discrete_survival(daily):
    ...
    if train.empty or test.empty:
        raise ValueError("Timeline split produced an empty train/test set")
...
# -----------------------------
def final_forecasts(
    seg, panel, daily, defects, tasks,
    model, columns, priors, duration_reg, duration_clf, dur_cols
):
...
def build_final_survival_curves(seg, daily, model, columns, horizon_days=30):
    """Return one row per segment/day with predicted survival S(t)."""
    final_month = daily["month"].max()
    latest = daily[daily["month"] == final_month].copy()

    rows = []
    for sid, g in latest.groupby("segment_id"):
        g = g.sort_values("date").head(horizon_days)
        hazards = predict_daily_hazard(model, columns, g)
        survival = survival_curve_from_hazards(hazards)

        for day_idx, (date, s) in enumerate(
            zip(g["date"].tolist(), survival.tolist()), start=1
        ):
            rows.append({
                "segment_id": sid,
                "forecast_as_of": str(pd.Timestamp(final_month).date()),
                "day": day_idx,
...
def main():
    ...
    # Full daily survival curve for the final forecast horizon.
    survival_curves = build_final_survival_curves(
        seg, daily, model, feature_cols, horizon_days=30
    )
